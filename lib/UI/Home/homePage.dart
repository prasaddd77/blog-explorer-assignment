                              ...
                              onTap: () {
                                Navigator.of(context).push(MaterialPageRoute(
                                    builder: (context) => BlogDetailPage(
                                          data: home.blogData.value[index],
                                          isFav: home.isFav.value[index],
                                        )));
                              },
                              child: Container(
                            ...
                            Positioned(
                              ...
                              child: ValueListenableBuilder(
                                  ...
                                  builder: (context, List<bool> fav, _) {
                                    return IconButton(
                                      onPressed: () async {
                                        home.isFav.value[index] =
                                            !home.isFav.value[index];
                                        home.isFav.notifyListeners();
                                        print(fav[index]);
                                        var res = await home.blogExist(
                                            "${home.blogData.value[index].id}");
                                        if (res == "yes") {
                                          home.favBlogs.value.removeWhere(
                                              (element) =>
                                                  element.id ==
                                                  home.blogData.value[index]
                                                      .id);
                                          ScaffoldMessenger.of(context)
                                              .showSnackBar(const SnackBar(
                                                  content: Text(
                                                      "Blog removed from favourite!"),
                                                  duration:
                                                      Duration(seconds: 1)));
                                        } else {
                                          home.favBlogs.value
                                              .add(home.blogData.value[index]);
                                          ScaffoldMessenger.of(context)
                                              .showSnackBar(const SnackBar(
                                            content: Text(
                                                "Blog added to favourite!"),
